...
<script setup>
    ...
    const { data:reviews } = await useAsyncData('reviews', async () => {
        const { data } = await client.from('reviews').select(`id, title, description, model, brand, slug, cover, type, created_at, published, product_title, profiles (username)`).eq('published', true).order('created_at')
